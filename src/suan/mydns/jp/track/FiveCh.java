...
public class FiveCh extends SuperTrack
{
	@Override
	public void Draw(Thoone th)
	{
		...
		for(int i = 0; i < 8; i++)
		{
			...
			if(DPCMWaveInport.WaveLoadFinish && System.currentTimeMillis() - DPCMWaveInport.finishtime < 1000) th.fill(0x00, 0xFF, 0x00);
			//if(MM2.old == i) th.fill(0xFF, 0x20, 0x00);
			th.rect(1280/24*(i+4), 0, 1280/24, 720/8);
			th.textSize(25);
			th.fill(0);
			th.text(Dutys[2][i], 1280/24*(i+4)+20, 720/8/2+10);
			if(th.kmState.MLeft)
			{
				if(th.kmState.IsMouseIn(1280/24*(i+4), 0, 1280/24, 720/8))
				{
					music = true;
					freq = i;
					th.mm2.ChStat(1, fr[2][i], 15, 0, 1, false, 1, 4, 16);
				}
			}
		}
		if(!music && th.kmState.IsMouseIn(1280/6, 0, 1280/6*2, 720/8))
		{
			if(!th.musics.start) th.mm2.ChStat(-1, 0.0f, 16, 0, 1, false, -1, 4, 16);
		}
	...
	protected void Onkai(Thoone th, int Channel)
	{
		...
		if(!music && th.kmState.IsMouseIn(1280/6, 720/4, 1280/6*5, 750/4*3))
		{
			this.Clicked = false;
			temp = R.nextInt(50)+2;
			if(!th.musics.start) th.mm2.ChStat(Channel == 3 ? 0 : -1, 0.0f, this.Vol, this.VolD, this.FreD, true, temp, Channel, this.VDUM);
		}
