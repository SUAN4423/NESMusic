...
public class PlayScreen extends PApplet
{
	...
	public void draw()
	{
		if(this.Visible && th != null)
		{
			...
			for(int i = 0; i < 4; i++)
			{
				if(temp[i] != (int)(MM2.Volumes[i] + (int)(MM2.Volumes[i/2] + ((int)((MM2.Dutys[i/2] == 1.0 ? 0.25 : MM2.Dutys[i/2]) * 4) << 6)) + (th.musics.nowNotes[i/2] << 8))) rewrite = true;
				...
				for(int i = 0; i < 8; i++)
				{
					if(i % 2 == 0)
					{
						try
						{
							...
							{
								temp[i/2] = (int)(MM2.Volumes[i/2] + ((int)((MM2.Dutys[i/2] == 1.0 ? 0.25 : MM2.Dutys[i/2]) * 4) << 6) + (th.musics.nowNotes[i/2] << 8));
